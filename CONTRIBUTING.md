...
└── docs/
    ├── harness/            # Dev methodology, backlog, task specs, QA reports
...
**`forgetty-vt`** — Safe Rust wrappers around libghostty-vt's C API. If you're adding a new terminal capability, this is where the FFI binding goes. Read `CLAUDE.md` → "FFI Pattern" before touching this crate — there are hard-won rules from weeks of debugging.
...
Forgetty uses a **harness methodology** where every feature goes through three phases:
...
This methodology is documented in `docs/harness/METHODOLOGY.md`. The current backlog lives in `docs/harness/BACKLOG.md`.
