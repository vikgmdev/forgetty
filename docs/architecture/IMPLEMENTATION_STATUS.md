...
| **AD-013** | Persistence = byte log, not cell snapshot | ✅ Implemented | V2-007 (2026-04-19). Append-only `~/.local/share/forgetty/logs/{pane_uuid}.log` replaces VT-state binary snapshots. In-memory ring (1 MiB default) + on-disk log (10 MiB cap, rotate-newest-half). Subscribe path does atomic subscribe+snapshot via `SessionManager::subscribe_with_snapshot` — client VT parser rebuilds state from replay bytes naturally, no cell-snapshot format. Cross-daemon orphan safety via `all_persisted_pane_ids()` union across all session JSONs (AD-001-aware). Daemon startup sync-saves session on every layout mutation to close the prune-race window. Seven fix cycles documented internally. **FIX-013 (2026-05-05, bundled with P-018)** fixed cold-restart replay: pre-fix, cold-restart minted fresh `Uuid::new_v4()` for every restored pane, leaving `logs/{old_uuid}.log` orphaned and the new pane reading an empty log. New `SessionManager::create_tab_with_pane_id` and `split_pane_with_ratio_and_pane_id` thread the saved pane_uuids from `PaneTreeState::Leaf::pane_id` (already in the schema with `#[serde(default)]`) through the cold-restart path so the on-disk log filename matches and `ByteLog::new`'s pre-load actually finds it. Pre-AD-016 the bug was masked by warm reattach; AD-016 unmasked it because every relaunch is now cold restart. |
| **AD-014** | Client-side color resolution | ✅ Implemented | V2-009 satisfied-by-construction, confirmed 2026-04-19. libghostty-vt stores cell colors as typed tags (`GhosttyStyleColor { tag: Palette\|Rgb\|None, ... }`), and the Rust-side `Terminal::sync_screen` resolves palette indices 0–15 via `self.ansi_palette[index]` on every frame read. `Terminal::set_ansi_palette` swaps the palette field; next `da.queue_draw()` re-resolves everything libghostty-vt currently surfaces (visible + scrollback accessed via viewport-scroll API). V2-007 byte-log replay populates libghostty-vt's tag state on restore, so recovered panes get retroactive theming too. Truecolor cells (`\x1b[38;2;r;g;b m`) correctly retain explicit RGB. No code change required; the BACKLOG-proposed "re-parse the byte log" alternative was defensive against a storage model libghostty-vt doesn't use. Verified by independent audit. |
| **AD-015** | `forgetty-sync` has no terminal deps | ✅ Implemented | V2-011 (2026-04-20). Terminal-specific iroh stream handler moved from `crates/forgetty-sync/src/stream.rs` (deleted, 635 lines) into `src/iroh_terminal.rs` under a lib+bin hybrid `forgetty-daemon` crate. `SyncEndpoint` gained a `builder().register_alpn(alpn, handler).build()` API; the daemon binary wires up the `forgetty/stream/1` ALPN via `register_alpn` while pairing (`forgetty/pair/1`) stays built-in as the reason `forgetty-sync` exists. `cargo tree -p forgetty-sync \| grep -E 'forgetty-(session\|pty\|vt\|workspace\|core)'` → zero hits (even `forgetty-core` removed — sync has no sibling-crate deps at all). MessagePack wire bytes byte-identical; 7 round-trip tests pin the format; all pre-existing invariants (authorization gate, 4 MiB frame cap, UUID pre-validation, no AD-009 hot-path polling) verified preserved by independent read. Verified by independent audit. |
| **AD-016** | Pinned-aware close; daemon does not survive | ✅ Implemented | P-018 (2026-05-04). Three-bucket on-disk layout (`sessions/`, `active/`, `trash/`) with one-shot migration gated on `.migration_p018` marker (`p018-v1`). Live daemons write `sessions/active/{uuid}.json` from startup; clean close moves it to `sessions/{uuid}.json` (pinned) or `sessions/trash/{uuid}.json` (unpinned), then `process::exit`. Crash recovery promotes pinned orphans, deletes unpinned ones. `set_pinned` RPC writes `active/{uuid}.json` synchronously before returning OK. `disconnect`/`shutdown_clean` paths converge on the same pinned-aware exit move. GTK shows an "Undo Close" notification on a sibling window when an unpinned session is closed and a sibling exists. Pin tooltip updated to "Pin to keep this window across launches". `--restore-session` reroutes `trash/` → `active/`. Amends AD-012. Bundles **FIX-013** so restored panes also see prior scrollback via byte-log replay (see AD-013 row). |
...
|----------|-------|---------|-----------|-----------|
| Architectural decisions (AD-001…AD-016) | 16 | 15 | 1 | 0 |
