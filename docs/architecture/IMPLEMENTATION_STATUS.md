...
| **AD-009** | No polling on the hot path | ❌ Not implemented | 20 ms drain loop in `daemon.rs`, 8 ms GLib timers in `terminal.rs`. **V2-001 (drain) + V2-002 (GTK) remove them.** |
...
|-----------|--------|-----------|
| 20 ms drain loop (`tokio::time::sleep` in `daemon.rs`) | ❌ To remove | V2-001 |
| 8 ms GLib timer on GTK PTY output poll | ❌ To remove | V2-002 |
